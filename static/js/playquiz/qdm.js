...
function time_update() {
    ...
    if (videoCurrentPos != pre) {
        ...
        //3 start
        else if (videoCurrentPos < 164) {
            current = 6666;
            currentNum = 2;
        }
        //3 end
        else if (videoCurrentPos < 183) {
            current = 3;
            currentNum = 3;
        }
        //4 start
        else if (videoCurrentPos < 202) {
            current = 6666;
            currentNum = 3;
        }
        //4 end
        else if (videoCurrentPos < 220) {
            current = 4;
            currentNum = 4;
        }
        //5 start
        else if (videoCurrentPos < 240) {
            current = 6666;
            currentNum = 4;
        }
        //5 end
        else if (videoCurrentPos < 257) {
            current = 5;
            currentNum = 5;
        }
        //6 start
        else if (videoCurrentPos < 293) {
            current = 6666;
            currentNum = 5;
        }
        //6 end
        else if (videoCurrentPos < 312) {
            current = 6;
            currentNum = 6;
        }
        //7 start
        ...
        //8 start
        else if (videoCurrentPos < 371) {
            current = 6666;
            currentNum = 7;
        }
        //8 end
        else if (videoCurrentPos < 389) {
            current = 8;
            currentNum = 8;
        }
        //9 start
        else if (videoCurrentPos < 409) {
            current = 6666;
            currentNum = 8;
        }
        //9 end
        else if (videoCurrentPos < 427) {
            current = 9;
            currentNum = 9;
        }
        //10 start
        else if (videoCurrentPos < 450 ) {
            current = 6666;
            currentNum = 9;
        }
        //아웃트로
        else if (videoCurrentPos < 474) {
            current = 10;
            currentNum = 10;
        }
        //정답공개
        else if (videoCurrentPos < 482) {
            current = 6666;
            currentNum = 10;
        }
        //정산 화면 출력 몇초 전
        else if(videoCurrentPos < 514){
            //결과체크를 미리 진행 하기 위해서.
        }
        // 결과 페이지로로가기전 상태
        else if (videoCurrentPos < 529) {
            current = 11;
            currentNum = 11;
        }
        //결과 페이지로
        ...
        if (last != current) {
            ...
            if (current == 6666 && currentNum != -1) {
                ...
                if( submissionObject.submission_answer > -1)
                    updateSubmission(stringToUtf8Bytes(JSON.stringify(submissionObject)))
